RoseCoin Core version 0.13.2 is now available from:

  <https://download.rosecoin.org/rosecoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/rosecoin-project/rosecoin/issues>
...
updates will be released anymore. Without security updates, using a rosecoin
...
ECDSA signatures inside RoseCoin transactions now use validation using
...
Previous versions of RoseCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of RoseCoin Core was to raise the default minimum
relay fee.

RoseCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
RoseCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running rosecoind without having to do any manual configuration.
...
RoseCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. RoseCoin Core will relay transactions with
...
entirely in RoseCoin Core 0.13, and it is at this time undecided whether the
...
RoseCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
RoseCoin Core automatically creates a hidden service to listen on, without
manual configuration. RoseCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if RoseCoin Core is listening, and
...
RoseCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals RoseCoin
Core to use floating fees. By default, RoseCoin Core will use floating
fees.
...
At all times, RoseCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ROC.
Furthermore, RoseCoin Core will never create transactions paying less than
...
`rosecoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 222123 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 222123 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /rosecoinrpc>
        ProxyPass http://127.0.0.1:22123/
        ProxyPassReverse http://127.0.0.1:22123/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass rosecoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< rosecoinrpc:<password>
    </Location>
...
Restarting rosecoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in rosecoin.conf
...
rosecoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/rosecoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The RoseCoin Core code base has
...
- `rosecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `rosecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the RoseCoin scripting system that in combination with
...
RoseCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of RoseCoin Core.
...
The two phases are distinct in the RoseCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming RoseCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for RoseCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each RoseCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of rosecoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of rosecoins being spent is exactly
...
- **Increased security for multisig:** RoseCoin addresses (both P2PKH addresses
  ...
  believed to be provided by RoseCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of RoseCoin's current ledger (called the UTXO set),
  ...
  note that RoseCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  RoseCoin users to individually opt-in to almost any change in the RoseCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and RoseCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the RoseCoin
client to RoseCoin Core 0.13.2 to signal for segwit support. When a
...
Since RoseCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `rosecoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `rosecoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
